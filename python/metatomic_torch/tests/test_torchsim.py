"""Tests for the inline MetatomicModel TorchSim wrapper.

Uses the metatomic-lj-test model so that tests run without needing metatrain or
downloading large model files. These tests cover the module at
metatomic.torch.torchsim (the inline wrapper shipped with metatomic-torch).
"""
...
def test_initialization_no_stress(lj_model):
    """Can disable stress computation."""
    model = MetatomicModel(model=lj_model, device=DEVICE, compute_stress=False)
    assert model.compute_forces is True
    assert model.compute_stress is False


def test_none_model_raises():
    """ValueError when model is None."""
    with pytest.raises(ValueError, match="model path"):
        MetatomicModel(model=None, device=DEVICE)


def test_bad_model_type_raises():
    """ValueError for unsupported model argument."""
    with pytest.raises(ValueError, match="AtomisticModel"):
