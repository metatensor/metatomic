...
# directly for its analytic values, and against the public O3Transformations for
...
class TestSymmetrizedModelForward:
    ...
    def test_energy_results_match_analytic_values_and_reuse_predictions(self):
        ...
        n_rotations = sum(batch.matrices.size(0) for batch in model._batches)
...
class TestSymmetrizedModelWrap:
    ...
    def test_preserves_requirements_and_runs_after_save_load(self, tmp_path):
        ...
        # the base model has no parameters, so the quadrature defaults to
        # float64; cast to match the model's declared float32 dtype
        wrapped.module.float()
        assert (
...
def test_decompose_quantity_commutes_with_o3(inversion, source_name, component_names):
    ...
    transformation = O3Transformations(
        (inversion * proper_rotation).unsqueeze(0),
