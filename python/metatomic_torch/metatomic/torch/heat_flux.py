...
class HardyHeatFluxWrapper(torch.nn.Module):
    ...
    def calc_hardy_heat_flux(self, system: System) -> torch.Tensor:
        ...
        masses: torch.Tensor = (
            system.get_data("masses").block().values.reshape(-1)
        )
        atomic_e = self._model([system], self.run_options, False)["energy"][
            0
        ].values.flatten()
        ...
        for i, energy in enumerate(atomic_e):
            hf_pot += (
                    wrap_positions(
                        system.positions[i] - system.positions, system.cell
                    )
                    * (
                        torch.autograd.grad(
                            energy, system.positions, retain_graph=True if i != len(atomic_e) - 1 else False
                        )[0]
                        * velocities
