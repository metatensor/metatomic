...
def load_system(file: str) -> System:
    ...
    if torch.jit.is_scripting():
        ...
        return _load_system_file(file)
    else:
        if isinstance(file, str):
            return _load_system_file(file)
        elif isinstance(file, pathlib.Path):
            return _load_system_file(str(file.resolve()))
        else:
...
def save(file: str, data: System) -> None:
    ...
    if torch.jit.is_scripting():
        assert isinstance(file, str)
        return _save_system_file(file, data)
    else:
        if isinstance(file, str):
            return _save_system_file(file, data)
        elif isinstance(file, pathlib.Path):
            return _save_system_file(str(file.resolve()), data)
        else:
            # assume a file-like object
            buffer = _save_system_buffer(data)
...
def save_buffer(data: System) -> torch.Tensor:
    """
    Save the given data (a :py:class:`System`) to an in-memory buffer, represented as a
    1-dimensional :py:class:`torch.Tensor` with ``uint8`` dtype.

    :param data: The :py:class:`System` object to serialize and save.
    """
    return _save_system_buffer(data)
