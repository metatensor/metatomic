...
class LibraryFinder:
    ...
    def __call__(self):
        if self._cached_dll is None:
            ...
            version = self._cached_dll.mta_version().decode("utf8")
...
def _lib_name():
    """Name of the metatomic shared library on the current platform"""
    if sys.platform.startswith("darwin"):
        return "libmetatomic.dylib"
    elif sys.platform.startswith("linux"):
        return "libmetatomic.so"
    elif sys.platform.startswith("win"):
        return "metatomic.dll"
    else:
...
def _lib_path():
    try:
        ...
        from ._external import EXTERNAL_METATOMIC_PATH
    ...
    raise ImportError("Could not find metatomic shared library at " + path)
...
def _check_dll(path):
    ...
    if python_machine == "x86":
        if machine != IMAGE_FILE_MACHINE_I386:
            raise ImportError("Python is 32-bit x86, but metatomic.dll is not")
    elif python_machine == "AMD64":
        if machine != IMAGE_FILE_MACHINE_AMD64:
            raise ImportError("Python is 64-bit x86_64, but metatomic.dll is not")
    elif python_machine == "ARM64":
        if machine != IMAGE_FILE_MACHINE_ARM64:
            raise ImportError("Python is 64-bit ARM, but metatomic.dll is not")
    else:
        raise ImportError(
            f"Metatomic doesn't provide a version for {python_machine} CPU. "
