...
from ._c_api import mta_status_t


class MetatomicError(Exception):
    """This class is used to throw exceptions for all errors in metatomic."""
...
def save_exception(e):
    """
    Save the given exception in meatomic's thread-local storage, so that it can be
...
def _get_exception(status=None):
    ...
    return MetatomicError(message.value.decode("utf8"), status=status)
