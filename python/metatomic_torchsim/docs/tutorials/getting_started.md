...
This tutorial walks through running a short NVE molecular dynamics simulation
with a metatomic model and TorchSim.
...
You also need a saved metatomic model file (`.pt`). If you have a metatrain
...
The wrapper detects the model's dtype and supported devices automatically. Pass
...
which serves as a basic sanity check for your model.

## Next steps

- {doc}`/howto/model_loading` covers all supported input formats
- {doc}`/howto/batched_simulations` explains running multiple systems at once
