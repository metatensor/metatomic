...
///
/// This checks that:
/// - all systems are on the same device and have the same dtype
/// - the systems device and dtype match what the model supports
...
pub(crate) fn check_inputs(
    ...
    let mut is_supported_device = false;
    for supported_device in &capabilities.supported_devices {
        if supported_device.is_valid(global_device) {
    ...
    if !is_supported_device {
        return Err(Error::InvalidParameter(format!(
            "the systems device ({}) is not supported by this model, only {:?} are",
            global_device, capabilities.supported_devices
        )));
    }
