...
struct AtomicTypes(Vec<i32>);

impl AtomicTypes {
    fn new(mut types: Vec<i32>) -> Self {
        types.sort_unstable();
        return AtomicTypes(types);
    }
}

impl std::ops::Deref for AtomicTypes {
    type Target = [i32];
...
static ATOMIC_TYPES_CACHE: LazyLock<Mutex<LruCache<AtomicTypes, ReferenceValue<i32>>>> = LazyLock::new(
...
pub fn execute_model(
    ...
    // TODO: measure the overhead of getting both of these information at every
    // model call. If it is too high, we can add a caching layer for them.
    ...
    // always check atomic types (even when check_consistency is false)
    {
        ...
        let atomic_types = AtomicTypes::new(capabilities.atomic_types.clone());
        let valid_types = cache.get_or_insert(atomic_types, || {
            ReferenceValue::new(
                ndarray::ArrayD::from_shape_vec(
                    ndarray::IxDyn(&[capabilities.atomic_types.len()]),
                    capabilities.atomic_types.clone(),
    ...
    // drop any outputs that where allocated so far
    let drop_partial_outputs = |outputs: *mut *mut mts_tensormap_t, outputs_count: usize| {
    ...
    if status != mta_status_t::MTA_SUCCESS {
        ...
        drop_partial_outputs(outputs, outputs_count);
