...
/// Check the layout of the "non_conservative_force" quantity.
pub(super) fn check(
    request: &Quantity,
    value: &TensorMap,
    systems: &[System],
    ...
    assert!(!request.name.is_custom() && request.name.base() == "non_conservative_force");
...
mod tests {
    ...
    fn valid_request() -> Quantity {
        Quantity {
            name: QuantityName::new("non_conservative_force".into()).unwrap(),
            unit: "eV/Angstrom".into(),
            description: None,
            gradients: vec![],
    ...
    fn valid_block() -> TensorBlock {
        ...
        let properties = Labels::new(["non_conservative_force"], [[0]]);
    ...
    fn valid_non_conservative_force() -> TensorMap {
    ...
    #[test]
    fn ok() {
        check(&valid_request(), &valid_non_conservative_force(), &[system(3)], None).unwrap();
    }
    ...
    fn empty_systems() {
        ...
        let non_conservative_force = TensorMap::new(Labels::new(["_"], [[0]]), vec![block]).unwrap();
        check(&valid_request(), &non_conservative_force, &[], None).unwrap();
        ...
        let non_conservative_force = TensorMap::new(Labels::new(["_"], [[0]]), vec![block]).unwrap();
        check(&valid_request(), &non_conservative_force, &[system(3)], None).unwrap();
    }
    ...
    fn selected_atoms() {
        ...
        let non_conservative_force = TensorMap::new(Labels::new(["_"], [[0]]), vec![block]).unwrap();
        check(&valid_request(), &non_conservative_force, &systems, Some(&selected_atoms)).unwrap();
    }
    ...
    fn multiple_systems() {
        ...
        let non_conservative_force = TensorMap::new(Labels::new(["_"], [[0]]), vec![block]).unwrap();
        check(&valid_request(), &non_conservative_force, &[system(3), system(1)], None).unwrap();
    }
    ...
    fn invalid_sample_kind() {
        ...
        let err = check(&request, &valid_non_conservative_force(), &[system(3)], None).unwrap_err();
        assert_eq!(
            err.to_string(),
            "invalid parameter: invalid sample_kind for 'non_conservative_force': expected one of [atom], got 'system'"
        );
    }

    #[test]
    fn wrong_number_of_blocks() {
        let non_conservative_force = TensorMap::new(Labels::empty(vec!["_"]), vec![]).unwrap();

        let err = check(&valid_request(), &non_conservative_force, &[system(3)], None).unwrap_err();
        assert_eq!(
            err.to_string(),
            "invalid parameter: invalid 'non_conservative_force': expected a single block, but found 0 blocks"
        );

        let non_conservative_force = TensorMap::new(
            Labels::new(["_"], [[0], [1]]),
        ...
        assert_eq!(
            err.to_string(),
            "invalid parameter: invalid 'non_conservative_force': expected a single block, but found 2 blocks"
        );
    }

    #[test]
    fn wrong_key() {
        let non_conservative_force = TensorMap::new(Labels::new(["foo"], [[0]]), vec![valid_block()]).unwrap();
        let err = check(&valid_request(), &non_conservative_force, &[system(3)], None).unwrap_err();
        assert_eq!(
            err.to_string(),
            "invalid parameter: invalid 'non_conservative_force': expected a single block with key '_', but found key names [foo]"
        );

        let non_conservative_force = TensorMap::new(Labels::new(["_"], [[1]]), vec![valid_block()]).unwrap();
        let err = check(&valid_request(), &non_conservative_force, &[system(3)], None).unwrap_err();
        assert_eq!(
            err.to_string(),
            "invalid parameter: invalid 'non_conservative_force': expected a single block with key value 0"
        );
    }
    ...
    fn wrong_property() {
        ...
        assert_eq!(
            err.to_string(),
            "invalid parameter: invalid properties for 'non_conservative_force': expected names [non_conservative_force], got [wrong]"
        );
        ...
        let block = TensorBlock::new(
            ...
            &Labels::new(["system", "atom"], [[0, 0], [0, 1], [0, 2]]),
            &[valid_xyz_component()],
            &Labels::new(["non_conservative_force"], [[1]]),
        ).unwrap();
        ...
        assert_eq!(
            err.to_string(),
            "invalid parameter: invalid properties values for 'non_conservative_force': expected [[0]]"
        );
    }
    ...
    fn missing_components() {
        ...
        assert_eq!(
            err.to_string(),
            "invalid parameter: invalid components for 'non_conservative_force': expected 1 component(s), got 0"
        );
    }
    ...
    fn wrong_component() {
        ...
        assert_eq!(
            err.to_string(),
            "invalid parameter: invalid components for 'non_conservative_force': expected names [xyz], got [abc]"
        );
        ...
        assert_eq!(
            err.to_string(),
            "invalid parameter: invalid components values for 'non_conservative_force': expected [[0], [1], [2]]"
        );
    }
    ...
    fn extra_component() {
        ...
        assert_eq!(
            err.to_string(),
            "invalid parameter: invalid components for 'non_conservative_force': expected 1 component(s), got 2"
        );
    }
    ...
    fn wrong_sample_names() {
        ...
        assert_eq!(
            err.to_string(),
            "invalid parameter: invalid sample names for 'non_conservative_force': expected [system, atom], got [system]"
        );
    }
    ...
    fn gradients() {
        ...
        assert_eq!(
            err.to_string(),
            "invalid parameter: invalid gradients for 'non_conservative_force': expected no gradients, but found gradients with respect to [positions]"
        );
    }
    ...
    fn selected_atoms_error() {
        ...
        let non_conservative_force = TensorMap::new(Labels::new(["_"], [[0]]), vec![block]).unwrap();
        let err = check(&valid_request(), &non_conservative_force, &[system(3)], Some(&selected_atoms)).unwrap_err();
        assert_eq!(
            err.to_string(),
            "invalid parameter: invalid samples for 'non_conservative_force', they do not match the `systems` and `selected_atoms`"
        );
    }
}
