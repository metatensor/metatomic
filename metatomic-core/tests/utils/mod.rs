...
/// Build metatomic-core located in `source_dir` inside `build_dir`, and return
/// the installation prefix
pub fn setup_metatomic_cmake(source_dir: &Path, build_dir: &Path, cmake_args: Vec<String>) -> PathBuf {
    std::fs::create_dir_all(build_dir).expect("failed to create metatomic build dir");

    // configure cmake for metatomic
    let mut cmake_config = cmake_config(source_dir, build_dir);
    ...
    run_command(cmake_config, "cmake configuration for metatomic");

    // build and install metatomic
    let mut cmake_build = cmake_build(build_dir);
    cmake_build.arg("--target");
    cmake_build.arg("install");

    run_command(cmake_build, "cmake build for metatomic");

    install_prefix
}

/// Install metatomic-core in a Python virtualenv with pip, and return the
/// CMAKE_PREFIX_PATH for the installed libmetatomic.
pub fn setup_metatomic_core_pip(python: &Path, source_dir: &Path) -> PathBuf {
    // build dependencies
    pip_install(
        python,
    ...
    // runtime dependencies which are not just metatensor and metatensor-torch
    pip_install(python, &["wigners"], PipInstallOptions::default());
    ...
    // let mut cmd = Command::new(python);
    // cmd.arg("-c");
    // cmd.arg("import metatomic; print(metatomic.utils.cmake_prefix_path)");

    // let output = run_command(cmd, "python to get metatomic cmake prefix");

    // let stdout = String::from_utf8_lossy(&output.stdout);
    // let prefix = PathBuf::from(stdout.trim());
    // if !prefix.exists() {
    //     panic!("'metatomic.utils.cmake_prefix' at '{}' does not exist", prefix.display());
    // }
