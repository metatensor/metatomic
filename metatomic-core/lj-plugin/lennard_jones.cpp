...
struct LennardJonesOptions {
  ...
  double epsilon = 1.0;
  double cutoff = 3.0;
  /// Atomic types reported in the model capabilities (not used to filter
  /// pairs).
...
/// Parse `atomic_type` as a single integer or a comma-separated list
/// (`"1,6,8"`).
std::vector<int32_t>
parse_atomic_types(const std::map<std::string, std::string> &options) {
  ...
  while (std::getline(in, token, ',')) {
    ...
    types.push_back(
        parse_int32(token.substr(first, last - first + 1), "atomic_type"));
  }
  if (types.empty()) {
    throw metatomic::Error(
        "Lennard-Jones option 'atomic_type' must be an integer");
...
system_energy_output(const std::vector<Calculation> &calculations,
...
metatensor::TensorMap
atom_energy_output(const std::vector<Calculation> &calculations,
                   const Selection &selection) {
  auto properties = metatensor::Labels({"energy"}, {{0}});
  std::vector<int32_t> samples;
  std::vector<double> energies;
...
public:
  ...
  std::vector<metatensor::TensorMap> execute_inner(
    ...
    for (const auto &output : requested_outputs) {
      ...
      outputs.push_back(system_energy_output(calculations, selection,
...
private:
  ...
  Calculation calculate(const metatomic::System &system,
    ...
    for (size_t pair = 0; pair < displacements.shape()[0]; pair++) {
      ...
      for (size_t xyz = 0; xyz < 3; xyz++) {
        ...
        result.positions_gradient[3 * static_cast<size_t>(first) + xyz] +=
            gradient;
        // The negative sign is for the equal and opposite force on the
        // second atom in the pair.
        result.positions_gradient[3 * static_cast<size_t>(second) + xyz] -=
            gradient;
      }
    }
