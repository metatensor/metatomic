...
# metatomic-torch, and copy it to the path given as argument
...
VERSION=$(cat "$ROOT_DIR/metatomic-torch/VERSION")
ARCHIVE_NAME="metatomic-torch-cxx-$VERSION"

TMP_DIR=$(mktemp -d)
mkdir "$TMP_DIR/$ARCHIVE_NAME"

cp -r "$ROOT_DIR"/metatomic-torch/* "$TMP_DIR/$ARCHIVE_NAME/"
...
cd "$ROOT_DIR"
./scripts/git-version-info.py "metatomic-torch-v" > "$TMP_DIR/$ARCHIVE_NAME/cmake/git_version_info"
