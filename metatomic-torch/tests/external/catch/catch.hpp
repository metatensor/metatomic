...
#define INTERNAL_CATCH_TEMPLATE_TEST_CASE_2(TestName, TestFunc, Name, Tags, Signature, ...)         \
    ...
    template <typename... Types> struct TestName {                                                  \
        TestName() {                                                                                \
            ...
            (void)expander{                                                                         \
                (reg_test(                                                                          \
                     Types{}, Catch::NameAndTags{Name " - " + std::string(tmpl_types[index]), Tags} \
                 ),                                                                                 \
                 index++)...                                                                        \
            }; /* NOLINT */                                                                         \
        }                                                                                           \
...
#pragma warning(                                                                                   \
    disable : 4180                                                                                 \
) // We attempt to stream a function (address) by const&, which MSVC complains about but is harmless
#endif
...
class XmlWriter {
  public:
    ...
    XmlWriter& startElement(
        std::string const& name, XmlFormatting fmt = XmlFormatting::Newline | XmlFormatting::Indent
    );
...
struct GeneratorTracker : TestCaseTracking::TrackerBase, IGeneratorTracker {
    ...
    GeneratorTracker(
        TestCaseTracking::NameAndLocation const& nameAndLocation, TrackerContext& ctx, ITracker* parent
    )
    ...
    void close() override {
        ...
        const bool should_wait_for_child = [&]() {
            ...
            if (std::find_if(
                    m_children.begin(), m_children.end(), [](TestCaseTracking::ITrackerPtr tracker) {
