...
NeighborListOptionsHolder::NeighborListOptionsHolder(
    double cutoff, bool full_list, bool strict, std::string requestor
)
    : cutoff_(cutoff), full_list_(full_list), strict_(strict) {
...
NeighborListOptions NeighborListOptionsHolder::from_json(const std::string& json) {
    ...
    if (data["class"] != "NeighborListOptions") {
        throw std::runtime_error(
            "'class' in JSON for NeighborListOptions must be 'NeighborListOptions'"
        );
    }
    ...
    if (data.contains("length_unit")) {
        if (!data["length_unit"].is_string()) {
            throw std::runtime_error(
                "'length_unit' in JSON for NeighborListOptions must be a string"
            );
        }
    ...
    if (data.contains("requestors")) {
        ...
        for (const auto& requestor : data["requestors"]) {
            if (!requestor.is_string()) {
                throw std::runtime_error(
                    "'requestors' in JSON for NeighborListOptions must be an array of strings"
                );
            }
...
torch::Tensor NeighborsAutograd::forward(
    ...
    if (check_consistency) {
        ...
        for (int64_t sample_i = 0; sample_i < samples.size(0); sample_i++) {
            ...
            auto cell_shift =
                samples.index({sample_i, torch::indexing::Slice(2, 5)}).to(positions.scalar_type());
            ...
            auto diff_norm =
                (actual_distance - expected_distance).norm().to(torch::kCPU).to(torch::kF64);
            if (diff_norm.item<double>() > epsilon) {
                std::ostringstream oss;

                oss << "checking internal consistency: one neighbor pair does not match its "
                       "metadata: ";
...
std::vector<torch::Tensor> NeighborsAutograd::backward(
    ...
    if (cell.requires_grad()) {
        auto cell_shifts = samples.index({torch::indexing::Slice(), torch::indexing::Slice(2, 5)})
                               .to(cell.scalar_type());
...
void metatomic_torch::register_autograd_neighbors(
    ...
    if (check_consistency) {
        if (system->positions().device() != distances.device()) {
            C10_THROW_ERROR(
                ...
                "got " +
                    system->positions().device().str() + " and " + distances.device().str()
            );
        }
        ...
        if (system->positions().scalar_type() != distances.scalar_type()) {
            C10_THROW_ERROR(
                ...
                "got " +
                    scalar_type_name(system->positions().scalar_type()) + " and " +
                    scalar_type_name(distances.scalar_type())
            );
        }
        ...
        if (neighbors->components().size() != 1 ||
            neighbors->components()[0]->names() != expected_names) {
            C10_THROW_ERROR(
                ValueError, "invalid `neighbors`: expected component names to be ['xyz']"
            );
        }
        ...
        if (neighbors->properties()->names() != expected_names) {
            C10_THROW_ERROR(
                ValueError, "invalid `neighbors`: expected property names to be ['distance']"
            );
        }
    }
    ...
    auto _ =
        NeighborsAutograd::apply(system->positions(), system->cell(), neighbors, check_consistency);
}
...
SystemHolder::SystemHolder(
    torch::Tensor types, torch::Tensor positions, torch::Tensor cell, torch::Tensor pbc
)
    : types_(std::move(types)), positions_(std::move(positions)), cell_(std::move(cell)),
      pbc_(std::move(pbc)) {
    if (positions_.device() != types_.device() || cell_.device() != types_.device() ||
        pbc_.device() != types_.device()) {
        C10_THROW_ERROR(
            ValueError,
    ...
    if (torch::canCast(types_.scalar_type(), torch::kInt32)) {
        ...
        C10_THROW_ERROR(
            ValueError,
            "`types` must be a tensor of integers, got " + scalar_type_name(types_.scalar_type()) +
                " instead"
        );
    }
    ...
    if (pbc_.size(0) != 3) {
        C10_THROW_ERROR(
            ValueError,
            "`pbc` must contain 3 entries, got a tensor with " + std::to_string(pbc_.size(0)) +
                " values"
        );
    }
    ...
    if (pbc_.scalar_type() != torch::kBool) {
        C10_THROW_ERROR(
            ValueError,
            "`pbc` must be a tensor of booleans, got " + scalar_type_name(pbc_.scalar_type()) +
                " instead"
        );
    }
...
void SystemHolder::set_types(torch::Tensor types) {
    if (types.device() != this->device()) {
        C10_THROW_ERROR(
            ValueError,
            "new `types` must be on the same device as existing data, got " + types.device().str() +
                " and " + this->device().str()
        );
    }
    ...
    if (types.size(0) != this->size()) {
        C10_THROW_ERROR(
            ValueError,
            "new `types` must contain " + std::to_string(this->size()) +
                " entries, "
                "got a tensor with " +
                std::to_string(types.size(0)) + " values"
        );
    }
    ...
    if (positions.scalar_type() != this->scalar_type()) {
        C10_THROW_ERROR(
            ...
            "new `positions` must have the same dtype as existing data, got " +
                scalar_type_name(positions.scalar_type()) + " and " +
                scalar_type_name(this->scalar_type())
        );
    }
...
void SystemHolder::set_cell(torch::Tensor cell) {
    if (cell.device() != this->device()) {
        C10_THROW_ERROR(
            ValueError,
            "new `cell` must be on the same device as existing data, got " + cell.device().str() +
                " and " + this->device().str()
        );
    }
    ...
    if (cell.scalar_type() != this->scalar_type()) {
        C10_THROW_ERROR(
            ...
            "new `cell` must have the same dtype as existing data, got " +
                scalar_type_name(cell.scalar_type()) + " and " +
                scalar_type_name(this->scalar_type())
        );
    }
...
void SystemHolder::set_pbc(torch::Tensor pbc) {
    if (pbc.device() != this->device()) {
        C10_THROW_ERROR(
            ValueError,
            "new `pbc` must be on the same device as existing data, got " + pbc.device().str() +
                " and " + this->device().str()
        );
    }
    ...
    if (pbc.scalar_type() != torch::kBool) {
        C10_THROW_ERROR(
            ValueError,
            "new `pbc` must be a tensor of booleans, got " + scalar_type_name(pbc.scalar_type()) +
                " instead"
        );
    }
    ...
    if (pbc.size(0) != 3) {
        C10_THROW_ERROR(
            ValueError,
            "new `pbc` must contain 3 entries, got a tensor with " + std::to_string(pbc.size(0)) +
                " values"
        );
    }
...
System SystemHolder::to(
    torch::optional<torch::Dtype> dtype, torch::optional<torch::Device> device
) const {
...
static std::tuple<torch::optional<torch::Dtype>, torch::optional<torch::Device>> to_arguments_parse(
    ...
    if (positional_1.isNone()) {
        ...
        C10_THROW_ERROR(
            TypeError, "unexpected type in " + context + ": " + positional_1.type()->str()
        );
    }
    ...
    if (positional_2.isNone()) {
        ...
        C10_THROW_ERROR(
            TypeError, "unexpected type in " + context + ": " + positional_2.type()->str()
        );
    }
...
System SystemHolder::to_positional(
    ...
    auto [parsed_dtype, parsed_device] =
        to_arguments_parse(positional_1, positional_2, dtype, device, "`System.to`");

    return this->to(parsed_dtype, parsed_device, non_blocking);
}

void SystemHolder::add_neighbor_list(
    NeighborListOptions options, metatensor_torch::TensorBlock neighbors
) {
    // check the structure of the NL
    auto samples_names = neighbors->samples()->names();
    if (samples_names.size() != 5 || samples_names[0] != "first_atom" ||
        samples_names[1] != "second_atom" || samples_names[2] != "cell_shift_a" ||
        samples_names[3] != "cell_shift_b" || samples_names[4] != "cell_shift_c") {
        C10_THROW_ERROR(
            ValueError,
    ...
    if (values.device() != this->device()) {
        C10_THROW_ERROR(
            ValueError,
            "`neighbors` device (" + values.device().str() +
                ") does not match "
    ...
    if (it != neighbors_.end()) {
        C10_THROW_ERROR(
            ValueError, "the neighbors list for " + options->str() + " already exists in this system"
        );
    }
...
metatensor_torch::TensorBlock SystemHolder::get_neighbor_list(NeighborListOptions options) const {
    auto it = neighbors_.find(options);
    if (it == neighbors_.end()) {
        C10_THROW_ERROR(
            ValueError,
            "No neighbor list for " + options->str() +
                " was found.\n"
...
static auto INVALID_DATA_NAMES = std::unordered_set<std::string>{
    "types", "positions", "position", "cell", "neighbors", "neighbor"
};
...
void SystemHolder::add_data(std::string name, metatensor_torch::TensorMap tensor, bool override) {
    ...
    if (INVALID_DATA_NAMES.find(string_lower(name)) != INVALID_DATA_NAMES.end()) {
        C10_THROW_ERROR(ValueError, "custom data can not be named '" + name + "'");
    }

    if (!override && data_.find(name) != data_.end()) {
        C10_THROW_ERROR(ValueError, "custom data '" + name + "' is already present in this system");
    }
    ...
    if (tensor->scalar_type() != this->scalar_type()) {
        C10_THROW_ERROR(
            ValueError,
            "dtype (" + scalar_type_name(tensor->scalar_type()) + ") of " + "custom data '" + name +
                "' does not match this system " + "dtype (" +
                scalar_type_name(this->scalar_type()) + ")"
        );
    }
...
metatensor_torch::TensorMap SystemHolder::get_data(std::string name) const {
    if (INVALID_DATA_NAMES.find(string_lower(name)) != INVALID_DATA_NAMES.end()) {
        C10_THROW_ERROR(ValueError, "custom data can not be named '" + name + "'");
    }
    ...
    if (it == data_.end()) {
        C10_THROW_ERROR(ValueError, "no data for '" + name + "' found in this system");
    }
    ...
    if (ALREADY_WARNED.insert(name).second) {
        TORCH_WARN(
            "custom data '",
            name,
            "' is experimental, please contact metatensor's ",
