...
static std::vector<uint8_t> build_header(const std::string& dict_ascii) {
  ...
  auto mk = [&](uint8_t major, uint8_t minor, size_t header_len_field_bytes) -> std::vector<uint8_t> {
    ...
    if (header_len_field_bytes == 2) {
      if (header_len > std::numeric_limits<uint16_t>::max()) return {};
      ...
      if (header_len > std::numeric_limits<uint32_t>::max()) return {};
      uint32_t hl = static_cast<uint32_t>(header_len);
      out.push_back(static_cast<uint8_t>(hl & 0xFF));
      out.push_back(static_cast<uint8_t>((hl >> 8) & 0xFF));
...
std::vector<uint8_t> npy_write(const torch::Tensor& t_in) {
  ...
  switch (dtype) {
    case torch::kFloat64: descr = "<f8"; itemsize = 8; break;
    case torch::kInt64:   descr = "<i8"; itemsize = 8; break;
    case torch::kInt32:   descr = "<i4"; itemsize = 4; break;
    case torch::kBool:    descr = "|b1"; itemsize = 1; break;
    default:
