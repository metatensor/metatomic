...
them.
    ...
    python export-models.py pet-mad-s-v1.5.0
    python export-models.py --all
    python export-models.py --all --relock
    python export-models.py --all --upload
    python export-models.py --check
...
VENVS_PATH = os.path.join(BUILD_PATH, "venvs")
...
def create_venv(name: str, dependencies: list, python: str) -> str:
    ...
    print(f"  creating a virtualenv with {python}")
...
def export(
    ...
    if locked and not relock and "spec" in locked:
        ...
        if spec != models_json:
            ...
            print("           pass --relock to build what models.json now describes")
    else:
    ...
    if locked and locked["sha256"] != digest:
        ...
        if locked["dependencies"] == installed:
            print(
                "    the dependencies are unchanged, so either the model does not\n"
                "    serialize deterministically, or an upstream package changed\n"
        ...
        if locked.get("url") is not None:
            print(
                "    dropping the 'url' in models.lock, it pointed at the previous\n"
                "    model: re-run with --upload to publish this one",
                file=sys.stderr,
            )
...
def check(name: str, models_json: dict, locked: dict) -> list:
    ...
    """
    if locked is None:
        return [(True, f"has never been built: run `export-models.py {name}`")]
    ...
    if "spec" not in locked:
        problems.append(
            (
                ...
                f"what it was built from: run `export-models.py {name} --relock`",
            )
        )
        ...
        problems.append(
            (
                ...
                f"`export-models.py {name} --relock`",
            )
        )
    ...
    if locked.get("url") is None:
        problems.append(
            (
                ...
                f"`export-models.py {name} --upload`",
            )
        )
    else:
...
def prune_builds(locked: dict):
    """
    Delete the built models that no entry of the lock refers to, so that a rebuild that
    does change a model does not leave the old one behind.
    """
